// LivingTree · Home — "THE LIVING TREE" (v7 · living tree + bottom dock)
// ---------------------------------------------------------------------------
...
const WAVE_SEEDS = [
  // crown top arc
  { x:706,  y:330, s:54 }, { x:884,  y:256, s:46 }, { x:1078, y:214, s:42 },
  { x:1288, y:202, s:48 }, { x:1500, y:220, s:44 }, { x:1704, y:262, s:46 },
  { x:1882, y:344, s:52 },
  // left shoulder
  { x:520,  y:438, s:50 }, { x:470,  y:582, s:44 }, { x:566,  y:690, s:40 },
  // right shoulder
  { x:2014, y:452, s:50 }, { x:2086, y:592, s:44 }, { x:1992, y:702, s:40 },
  // mid-canopy accents
  { x:820,  y:470, s:38 }, { x:1156, y:382, s:36 }, { x:1444, y:404, s:38 },
  { x:1742, y:492, s:36 },
  // lower canopy
  { x:946,  y:642, s:42 }, { x:1300, y:664, s:40 }, { x:1622, y:642, s:42 },
];
...
function Home() {
  ...
  // canopy bend — leans from the trunk base
  const canopyLean = Math.sin(t * 0.42) * 1.0 + peak * 2.0 * Math.sin(t * 0.74 + 1.3);
  ...
  // tree image geometry — REDUCED from full-bleed; centred, feathered edges
  const TREE_W = 2080, TREE_H = Math.round(2080 * 1428 / 2560), TREE_TOP = 150;
  ...
  return (
    <div ref={stageRef} onClick={handleStageClick} style={{
      ...
      {/* meadow band the reduced tree stands on (between tree base and dock) */}
      <div style={{ position:'absolute', left:0, right:0, top:1140, height:280, pointerEvents:'none',
        background:'linear-gradient(180deg, rgba(214,206,165,0) 0%, #d4cea1 55%, #cdc79d 100%)' }}/>

      {/* ====== SCENE: the reduced painted tree, bending in the wind, with
      ...
      <div style={{
        ...
        {/* the painted tree — reduced, edges feathered so the rectangle
            dissolves into the sky wash */}
        <img src="assets/brand/hometree.png" alt=""
          style={{
            ...
            WebkitMaskImage:'radial-gradient(ellipse 84% 92% at 50% 44%, #000 64%, transparent 100%)',
            maskImage:'radial-gradient(ellipse 84% 92% at 50% 44%, #000 64%, transparent 100%)',
          }}/>
