    ...
    <div style={{position:'absolute', inset:0, background:'transparent', color:'var(--ink)', overflow:'hidden'}}>
      ...
      <div style={{position:'absolute', bottom:dlerp(dens,48,56), left:60, right:60, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        ...
        <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:dlerp(dens,11,14)}}>
          <div className="mono" style={{fontSize:dlerp(dens,17,19), letterSpacing:'0.34em', color:'var(--slate)'}}>STEP {step} OF 4</div>
      ...
      <div style={{position:'absolute', inset:0, zIndex:60, display:'flex', alignItems:'center', justifyContent:'center',
        ...
        <div style={{textAlign:'center', maxWidth:780, padding:'58px 66px', borderRadius:26,
          ...
          <div className="mono" style={{fontSize:17, letterSpacing:'0.36em', color:'var(--gold-deep)'}}>MODULE · 08 / BOOKING</div>
...
function ScoreReadout({ step, score }) {
  ...
  return (
    <div style={{display:'flex', alignItems:'center', gap:13}}>
      ...
      <div className="mono" style={{fontSize:17, letterSpacing:'0.24em', color:'var(--gold-deep)', minWidth:57, textAlign:'left'}}>{pct}%</div>
    </div>
  );
}
    ...
    <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:dlerp(d,53,80), height:'100%', alignItems:'stretch'}}>
      ...
      <div style={{display:'flex', flexDirection:'column', justifyContent:'center', gap:dlerp(d,32,44)}}>
        ...
        <div style={{display:'flex', flexDirection:'column', gap:13, padding:dlerp(d,25,32)+'px '+dlerp(d,29,34)+'px', background:'var(--ivory-2)', border:'1px solid var(--line)', borderRadius:15}}>
          ...
          <div style={{fontSize:dlerp(d,24,28), color:'var(--graphite)', lineHeight:1.55}}>
      ...
      <div style={{display:'flex', flexDirection:'column', justifyContent:'center', gap:dlerp(d,22,30), paddingTop:dlerp(d,6,18), paddingBottom:dlerp(d,6,18)}}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:19, padding:dlerp(d,21,28)+'px '+dlerp(d,25,30)+'px', borderRadius:15, background:'var(--ivory-2)', border:'1px solid var(--line)'}}>
          ...
          <label htmlFor="loan" className="serif" style={{fontSize:dlerp(d,24,28)}}>I have a home-loan pre-approval</label>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{display:'grid', gridTemplateColumns:'1fr 1.1fr', gap:dlerp(d,53,72), height:'100%', alignItems:'center'}}>
      <div style={{display:'flex', flexDirection:'column', justifyContent:'center', gap:dlerp(d,32,42)}}>
        ...
        <div key={`preview-${pick.block}-${pick.towerId}`} style={{
          ...
          <div className="mono" style={{fontSize:dlerp(d,17,18), letterSpacing:'0.32em', color:'rgba(250,246,232,0.7)'}}>SELECTED</div>
          ...
          <div style={{display:'flex', gap:dlerp(d,32,44), marginTop:dlerp(d,21,28), flexWrap:'wrap'}}>
            ...
            <div>
              <div className="mono" style={{fontSize:dlerp(d,17,17), letterSpacing:'0.28em', color:'rgba(250,246,232,0.55)'}}>CLUSTER</div>
      ...
      <div style={{display:'flex', flexDirection:'column', justifyContent:'center', gap:dlerp(d,25,34)}}>
        ...
        <div>
          <div className="mono" style={{fontSize:dlerp(d,17,18), letterSpacing:'0.28em', color:'var(--slate)'}}>PREFERRED TOWER</div>
            ...
            {TOWERS.map(t => {
              ...
              return (
                <button key={t.id}
                  ...
                  <div className="mono" style={{fontSize:dlerp(d,17,17), letterSpacing:'0.2em', color: sel ? 'rgba(250,246,232,0.85)' : 'var(--slate)'}}>{towerSummary(t.id).available} avail</div>
                  {sel && (
                    <div style={{
      ...
      <div style={{display:'flex', flexDirection:'column', justifyContent:'center'}}>
        ...
        <div style={{
          ...
          <div style={{display:'flex', flexDirection:'column', gap:6}}>
            ...
            <div className="mono" style={{fontSize:dlerp(D,17,18), letterSpacing:'0.24em', color:'rgba(250,246,232,0.7)'}}>NEHRU NAGAR · AHMEDABAD · 380015</div>
          </div>
        </div>
      </div>

      <div style={{display:'flex', flexDirection:'column', justifyContent:'center', gap:dlerp(D,25,32), height:'100%'}}>
        <div className="mono" style={{fontSize:dlerp(D,17,18), letterSpacing:'0.28em', color:'var(--slate)'}}>PICK A DATE</div>
          ...
          {days.map((d) => {
            ...
            return (
              <button key={key} onClick={(ev)=>onDateTap(ev, key)} style={{
                ...
                <div className="mono" style={{fontSize:dlerp(D,17,17), letterSpacing:'0.22em', color: sel ? 'rgba(250,246,232,0.7)' : 'var(--slate)'}}>{d.toLocaleDateString('en',{weekday:'short'}).toUpperCase()}</div>
                <div className="serif" style={{fontSize:dlerp(D,38,46), fontWeight:400, lineHeight:1}}>{d.getDate()}</div>
                <div className="mono" style={{fontSize:dlerp(D,17,17), letterSpacing:'0.18em', color: sel ? 'rgba(232,215,168,0.85)' : 'var(--muted)'}}>{d.toLocaleDateString('en',{month:'short'}).toUpperCase()}</div>
              </button>
            );
          })}
        </div>

        <div className="mono" style={{fontSize:dlerp(D,17,18), letterSpacing:'0.28em', color:'var(--slate)', marginTop:dlerp(D,15,20)}}>PREFERRED TIME</div>
        ...
        <div style={{display:'flex', flexDirection:'column', flex:'1 1 auto', minHeight:dlerp(D,150,180)}}>
          <div className="mono" style={{fontSize:dlerp(D,17,18), letterSpacing:'0.28em', color:'var(--slate)'}}>SPECIAL REQUESTS (OPTIONAL)</div>
...
function Detail({ k, v, sub, dens = 0 }) {
  ...
  return (
    <div>
      <div className="mono" style={{fontSize:dlerp(d,17,18), letterSpacing:'0.32em', color:'var(--slate)'}}>{k}</div>
      <div className="serif" style={{fontSize:dlerp(d,34,42), fontWeight:400, marginTop:11, letterSpacing:'-0.005em'}}>{v}</div>
      {sub && <div className="mono" style={{fontSize:dlerp(d,17,18), letterSpacing:'0.22em', color:'var(--gold-deep)', marginTop:8, textTransform:'uppercase'}}>{sub}</div>}
    </div>
  );
}
...
function Field({ label, v, set, span = 1, dens = 0 }) {
  ...
  return (
    <div style={{gridColumn: 'span ' + span, position:'relative'}}>
      <div className="mono" style={{
        fontSize:dlerp(d,17,18), letterSpacing:'0.28em',
