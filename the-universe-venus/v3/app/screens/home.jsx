...
function Home() {
  ...
  const dens = Math.max(clamp((H - 1600) / 320), (typeof window!=="undefined" && window.UNIVERSE_CANVAS && window.UNIVERSE_CANVAS.dens) || 0);
  ...
  const [crmGate, setCrmGate] = React.useState(false);   // 4-digit agent sign-in
  const [agent, setAgent]     = React.useState(null);     // { code, name } once connected
  ...
  const handleCenterTap = () => {
    ...
    if (s.n >= 3) {
      s.n = 0;
      // Open the mini-CRM behind a personal sign-in: triple-tap → code gate.
      if (crmOpen) { setCrmOpen(false); setAgent(null); }
      else setCrmGate(g => !g);
      return;
    }
  ...
  return (
    <div style={{position:'absolute', inset:0, background:'transparent', color:'var(--ink)', overflow:'hidden',
      ...
      {crmGate && <AgentCodeGate
        onCancel={() => setCrmGate(false)}
        onSignIn={(ag) => { setAgent(ag); setCrmGate(false); setCrmOpen(true); }}/>}
      {crmOpen && <SalesDesk agent={agent} onClose={() => { setCrmOpen(false); setAgent(null); }}/>}
    </div>
  );
}
...
function AgentCodeGate({ onCancel, onSignIn }) {
  ...
  React.useEffect(() => {
    if (!document.getElementById('crm-gate-kf')) {
      const s = document.createElement('style'); s.id = 'crm-gate-kf';
      s.textContent = '@keyframes crmShake{0%,100%{transform:translateX(0)}20%,60%{transform:translateX(-11px)}40%,80%{transform:translateX(11px)}}';
      document.head.appendChild(s);
    }
  }, []);
  ...
  const press = (d) => {
    ...
    const next = code + d; setCode(next);
    if (next.length === 4) submit(next);
  };
  const back = () => { if (!busyRef.current) setCode(c => c.slice(0, -1)); };
  ...
  return (
    <div data-crm="1" style={{position:'absolute', inset:0, zIndex:80, display:'flex', alignItems:'center', justifyContent:'center',
      background:'radial-gradient(120% 120% at 50% 38%, rgba(26,19,9,0.88), rgba(11,8,4,0.95))', backdropFilter:'blur(12px)'}}>
      <button onClick={onCancel} aria-label="Cancel" className="mono" style={{position:'absolute', top:44, right:52, width:60, height:60,
        borderRadius:'50%', border:'1px solid rgba(232,215,168,0.34)', background:'rgba(255,255,255,0.06)', color:'var(--gold-soft)',
        fontSize:26, cursor:'pointer', lineHeight:1}}>×</button>
      <div style={{width:560, textAlign:'center', color:'#f4ecd8'}}>
        <div className="mono" style={{fontSize:17, letterSpacing:'0.44em', color:'var(--gold-soft)', textTransform:'uppercase'}}>Sales Desk · Sign in</div>
        <h2 className="serif" style={{margin:'16px 0 6px', fontSize:60, fontWeight:300, letterSpacing:'-0.02em', color:'#fff7e6', lineHeight:1}}>Agent code</h2>
        <div style={{fontSize:22, fontStyle:'italic', color:'rgba(240,232,214,0.7)', marginBottom:38}}>Enter your 4-digit code to connect</div>
        <div style={{display:'flex', justifyContent:'center', gap:24, marginBottom:44, animation: err ? 'crmShake 500ms' : 'none'}}>
          {[0,1,2,3].map(i => {
            const on = i < code.length;
            return <div key={i} style={{width:70, height:86, borderRadius:16,
              border:'2px solid '+(err ? 'var(--venus-red)' : on ? 'var(--gold)' : 'rgba(232,215,168,0.3)'),
              background: on ? 'rgba(232,215,168,0.14)' : 'rgba(255,255,255,0.04)',
              display:'flex', alignItems:'center', justifyContent:'center', fontSize:46, color:'#fff7e6',
              transition:'border-color 200ms, background 200ms'}}>{on ? '•' : ''}</div>;
          })}
        </div>
        <div style={{display:'grid', gridTemplateColumns:'repeat(3, 104px)', gap:18, justifyContent:'center'}}>
          {keys.map((k, i) => k === '' ? <div key={i}/> :
            <button key={i} onClick={() => k === '⌫' ? back() : press(k)} className={k === '⌫' ? 'mono' : 'serif'} style={{
              width:104, height:96, borderRadius:18, cursor:'pointer',
              border:'1px solid rgba(232,215,168,0.28)', background:'rgba(255,255,255,0.05)',
              color:'#f4ecd8', fontSize: k === '⌫' ? 30 : 40, fontWeight:300, transition:'background 160ms, transform 120ms'}}
              onMouseDown={(e) => e.currentTarget.style.transform = 'scale(0.94)'}
              onMouseUp={(e) => e.currentTarget.style.transform = 'scale(1)'}
              onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(232,215,168,0.16)'}
              onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }}
            >{k}</button>)}
        </div>
        <div className="mono" style={{marginTop:32, fontSize:15, letterSpacing:'0.2em', textTransform:'uppercase',
          color: err ? 'var(--venus-red)' : 'rgba(240,232,214,0.42)'}}>
          {err ? 'Code not recognised — try again' : 'Each salesperson has a personal code'}
        </div>
      </div>
    </div>
  );
        ...
        <div style={{display:'flex', alignItems:'center', gap:12}}>
          {agent && (
            <div className="mono" style={{display:'flex', alignItems:'center', gap:10,
              background:'rgba(176,138,63,0.14)', border:'1px solid rgba(176,138,63,0.4)', borderRadius:999,
              padding:'12px 20px', fontSize:12.5, letterSpacing:'0.16em', color:'var(--gold-deep)'}}>
              <span style={{width:9, height:9, borderRadius:'50%', background:'var(--gold)', boxShadow:'0 0 10px rgba(212,175,90,0.9)'}}/>
              CONNECTED · {agent.name.toUpperCase()}
            </div>
          )}
