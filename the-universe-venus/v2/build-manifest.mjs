...
const SKIP_DIR = new Set(['plans_orig', '_plate_src_backup', '_build-docs', '_massing3d', 'node_modules', '.git']);
...
// Never precache dev backups/scratch: `.bak.`, `.orig.`, `.pre<digits>.` snapshots.
const SKIP_MATCH = (name) => name.includes('.bak.') || name.includes('.orig.') || /\.pre\d+\./.test(name);
