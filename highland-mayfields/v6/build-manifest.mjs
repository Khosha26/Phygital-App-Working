...
function isExcluded(rel, base) {
  ...
  // dead assets — only referenced by excluded scratch home-v* files, or unused sources
  if (parts.includes('plates') || parts.includes('buttons')) return true;
  if (base === 'home-day.jpg' || base === 'home-night.jpg') return true;
  if (base === 'bg-towers.png') return true;
  if (base === 'aerial-estate@2560.jpg') return true;
  if (base === 'inv-site@3840.webp') return true;
  // raw source villa SVGs (5MB embedded rasters; app uses extracted webp + region-data.js)
  if (base === 'sky-villa-img.svg' || base === 'sky villa-floors.svg' || base === 'sky-villa-skech.svg') return true;
  return false;
}
